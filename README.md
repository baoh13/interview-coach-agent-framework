...
- [Workflow bundle overview](workflow/README.md)
- Use workflow prompts in [workflow/prompts/](workflow/prompts/)
- Use workflow skills in [workflow/skills/](workflow/skills/)

### Bundles

Available bundles in this repository are in [bundles/](bundles/). Bundles package reusable agents, prompts, and skills for specific workflows.

- [repository-manager](bundles/repository-manager/README.md): Multi-repo repository onboarding and catalog management. Use its prompts in [bundles/repository-manager/prompts/](bundles/repository-manager/prompts/) (for example `onboard-repo.prompt.md` and `reset-repos.prompt.md`) and its management skill in [bundles/repository-manager/skills/repository-management/SKILL.md](bundles/repository-manager/skills/repository-management/SKILL.md).
- [visual-explainer](bundles/visual-explainer/README.md): Generate browser-based visual explainers (architecture views, diff reviews, plans, recaps). Use its prompts in [bundles/visual-explainer/prompts/](bundles/visual-explainer/prompts/) such as `diff-review.prompt.md`, `plan-review.prompt.md`, and `project-recap.prompt.md`.

How bundles are used:

1. Select the bundle based on the task type (repository operations vs visual analysis).
2. Invoke the relevant prompt for one-off tasks.
3. Let the prompt activate the bundle skill when a multi-step workflow is needed.
