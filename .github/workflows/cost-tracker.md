...
on:
  workflow_run:
    workflows: ["daily-repo-status"]  # Edit to match your agent workflow names
    types:
      - completed
...
- **Triggered by**: ${{ github.event.workflow_run.number }}
...
| | |
|---|---|
| **Workflow** | ${{ github.event.workflow_run.number }} |
| **Run** | [#${{ github.event.workflow_run.run_number }}](${{ github.event.workflow_run.html_url }}) |
| **Conclusion** | ${{ github.event.workflow_run.conclusion }} |
...
title format:
`[cost-tracker] ${{ github.event.workflow_run.number }} #${{ github.event.workflow_run.run_number }}: $TOTAL_COST`
...
`[cost-tracker] High spend alert: $TOTAL_COST for ${{ github.event.workflow_run.number }}`
