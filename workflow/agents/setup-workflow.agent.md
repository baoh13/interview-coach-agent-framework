...
# Installed bundles
- `repository-manager` - check if `repository-manager` agent is installed and bundle `repository-manager` directory is present in the `bundles` directory.
- `visual-explainer` - check if the `visual-explainer` skill is installed and bundle `visual-explainer` directory is present in the `bundles` directory.